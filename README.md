...
聚焦电商场景咨询意图杂、PDF 商品和规则手册信息提取难以及核心业务办理依赖人工等痛点，搭建全栈电商智能客服中台。系统采用 **"意图拦截分流+ Context 预注入+ Agent 推理兜底"** 架构，实现简单意图秒回、意图识别触发下的多路业务数据预取及 Agent 复杂场景自适应推理闭环。项目构建起从多模态数据解析、多路混合检索到自动量化评测的完整链路，显著提升复杂咨询环境下的响应速度与回复精度。
